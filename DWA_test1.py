...
class DWA:
    ...
    def lds_callback(self, scan):
        ...
        if np.hypot(goal_pos[0] - self.waffle_pose.x, goal_pos[1] - self.waffle_pose.y) <= 0.2:
            ...
            best_score = self.ww_evaluate_scores(pos_candidates, goal_pos, scan_range, local_pos)
            turtle_vel.linear.x, turtle_vel.angular.z = mps[best_score[0]], radps[best_score[1]]
            self.move_pos(best_score, local_pos)
    ...
    def ww_evaluate_scores(self, pos_candidates, goal_pos, data_array, local_pos):         
        remaining_scores = self.find_remaining_scores(pos_candidates, goal_pos)
        obstacle_scores = self.find_others_scores(data_array, local_pos)
        scores = 1.7 * remaining_scores - 1.3 * obstacle_scores
        best_score = np.unravel_index(np.argmin(scores), scores.shape)
        return best_score
