...
class SelfDrive:
    def __init__(self, publisher):
        ...
        self.save_x = -1.979
        self.save_y = 0.0
        self.angle_160 = np.arange(-60, 60).reshape(4, 30)
    ...
    def obstacle_scoring(self):
        ...
        side_left = np.logical_and(0.6 > data[0] >= 0.2, data[2] > data[1])
        side_left2 = np.logical_and( 0.6 > data[1] >= 0.2, data[2] > data[1])
        
        side_right = np.logical_and(0.6 > data[3] >= 0.2, data[2] < data[1])
        side_right2 = np.logical_and(0.6 > data[2] >= 0.2, data[2] < data[1])     
        
        front = np.logical_and(0.6 > np.mean(data[1:3]) >= 0.2, data[0] < data[3])    #turn left   
        front_1 = np.logical_and(0.6 > np.mean(data[1:3]) >= 0.2, data[0] > data[3])  #turn right          

        #if back == True:           
        #    rospy.loginfo("back")
        #    return -0.2, self.radps[self.best_score[1]]
        if side_left == True or side_left2 == True or front == True: # 우회전
            rospy.loginfo("turn left")
            print(f"{self.best_score[0]}, {data[0]}, {data[1]}, {data[2]}, {data[3]}")
            return self.mps[self.best_score[0]], -0.5
        elif side_right == True or side_right2 == True or front_1 == True: # 좌회전
